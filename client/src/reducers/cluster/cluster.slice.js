...
export const accessLoginUserCluster = createAsyncThunk("access/cluster", async (clusterDetails, thunkAPI) => {
    try {
        const { id, password } = clusterDetails;
        const token = thunkAPI.getState().auth.user.token;
        return await clusterService.accessMyCluster(id, password, token);
    } catch (error) {
        const message = (error.response && error.response.data && error.response.data.message)
            || error.message || error.toString();
        return thunkAPI.rejectWithValue(message)
...
const clusterSlice = createSlice({
    ...
    extraReducers: builder => {
        builder
            ...
            .addCase(accessLoginUserCluster.pending, state => {
                state.isLoading = true;
            })
            .addCase(accessLoginUserCluster.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.cluster = action.payload.cluster
            })
            .addCase(accessLoginUserCluster.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.message = action.payload;
            })
